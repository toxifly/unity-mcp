...
@pytest.mark.asyncio
async def test_http_preserves_plugin_hub_queue_metadata(monkeypatch):
    """HTTP unwrapping exposes queue diagnostics like the legacy transport."""
    monkeypatch.setattr(config, "transport_mode", "http")
    monkeypatch.setattr(config, "http_remote_hosted", False)
    ...
    result = await unity_transport.send_with_unity_instance(
        _unused_send_fn, None, "manage_scene", {"action": "get_active"}
    )
