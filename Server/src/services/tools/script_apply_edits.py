...
def _infer_class_name(script_name: str) -> str:
    # Partial-class files conventionally use "Type.Part.cs" while still
    # declaring Type. Match ManageScript.TypeNameFromFileStem by taking the
    # first segment instead of searching C# for the full dotted file stem.
    return (script_name or "").strip().split(".", 1)[0]
...
@mcp_for_unity_tool(
    ...
    description=(
        "Apply mutating, structured C# method or anchor edits with boundary validation. edits accepts a "
        "list or JSON string using replace_method, insert_method, delete_method, anchor_insert, "
        "anchor_delete, or anchor_replace. Method operations use className, methodName, replacement, "
        "position, afterMethodName, and beforeMethodName as applicable; anchor operations use anchor and "
        "text. options controls validation and refresh behavior. Prefer standard validation and stable "
        "anchors; structural guards reject edits that unbalance braces. Use this tool instead of raw text "
        "for whole-method changes."
    ),
