...
from services.tools.utils import parse_json_payload, normalize_properties, coerce_bool, coerce_int
from services.tools.preflight import preflight


@mcp_for_unity_tool(
    description="Manages components on GameObjects. Actions: add, remove, set_property, get_properties, get_components."
)
async def manage_components(
    ...
    include_properties: Annotated[bool | str, "For get_components: include serialized component properties (default true)."] | None = None,
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        if action == "get_components":
            ...
            include_props_val = coerce_bool(include_properties, default=True)
            if include_props_val is None:
                include_props_val = True
