...
4. **Expect tool visibility preferences to migrate.** Tool visibility now uses
   group-based defaults, and the Unity package stores only explicit per-tool
   choices. Explicit disables from v9 are preserved; legacy enables are
   indistinguishable from v9's auto-initialised defaults and are discarded.
   Optional (non-core) groups therefore start hidden after upgrading — re-enable
   them with `manage_tools` or the Editor's Tools tab if you had them on.
5. **Enable `asset_gen` explicitly** with `manage_tools` before calling asset
   generation tools.
6. **Add provider keys in Unity**, not in MCP client config files.
7. **Keep generated assets under review** before committing them. Provider
